...
| S→C | `{t:'pos', id, z, lane, x, spd, score, st}` | | 打上 id 转发给其他所有人（不含 seq） |
| C→S | `{t:'crash', z, lane}` | | 撞墙眩晕开始（**不淘汰**） |
| S→C | `{t:'crash', id, z}` | | 广播给其他人（幽灵车闪烁表现） |
| C→S | `{t:'finish', score}` | | 本地 z ≥ len。早于理论最快时间（len/53.3 秒 × 0.9）判非法忽略 |
| S→C | `{t:'finish', id, rank, time, score}` | | time 为 startAt 起算毫秒。同 100ms 窗内按 score 排名次 |
